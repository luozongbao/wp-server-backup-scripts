...
show_help() {
    ...
    echo "  $0 -b /backups/20250530_143022_webserver_backup.zip"
    echo ""
    echo "  # Restore to a specific native path"
    echo "  $0 -b /backups/20250530_143022_webserver_backup.zip -f /etc/nginx"
    echo ""
    echo "  # Restore to a Docker container"
    echo "  $0 -b /backups/20250530_143022_webserver_backup.zip -c my_nginx_container"
    echo ""
    echo "  # Restore to a specific in-container path"
    echo "  $0 -b /backups/20250530_143022_webserver_backup.zip -c my_nginx_container -f /etc/nginx"
    echo ""
    echo "  # Preview first"
    echo "  $0 -b /backups/20250530_143022_webserver_backup.zip --dry-run"
    echo ""
    echo "  # Force restore (skip safety backup of existing config)"
    echo "  $0 -b /backups/20250530_143022_webserver_backup.zip --force"
    echo ""
    echo "Features:"
